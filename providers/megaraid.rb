...
action :createspan do
  available = true
  new_resource.pds.each do |pds|
    unless pdsavailable(pds)
      Chef::Log::warn("Skipping creation of span due to physical disk already in use (already configured?)")
      available = false
    end
  end
  a = 0 # track array number
  if available
    arraystring = ""
    new_resource.pds.each do |pds|
      pdlist = pds.join(',')
      arraystring = "#{arraystring} -Array#{a} \\[#{pdlist}\\] "
      a = a+1
    end
    execute "create span" do
      command "/opt/MegaRAID/MegaCli/MegaCli64 CfgSpanAdd -R#{new_resource.level} #{arraystring}WB RA -strpsz64 -a#{new_resource.controller}"
    end
  end
end
